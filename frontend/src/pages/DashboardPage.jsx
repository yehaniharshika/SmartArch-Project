...
import {
  ...
  FileImage, TrendingUp, DoorOpen, Square,
} from "lucide-react";
...
const MOCK_PLANS = [
  ...
  },
  {
    ...
    wall_count: 10, door_count: 4, window_count: 6, room_count: 4,
  ...
  },
  {
    ...
    wall_count: 0, door_count: 0, window_count: 0, room_count: 0,
    share_token: null,
  ...
  },
  {
    ...
    wall_count: 14, door_count: 6, window_count: 9, room_count: 5,
...
export default function DashboardPage() {
  ...
  const filtered = plans.filter((p) => {
    const matchSearch = p.original_filename.toLowerCase().includes(search.toLowerCase());
    const matchFilter = filter === "all" || p.status === filter;
    return matchSearch && matchFilter;
  });

  const readyPlans = plans.filter((p) => p.status === "ready");
  const totalArea  = readyPlans.reduce((s, p) => s + (p.total_floor_area_sqm || 0), 0);
      ...
      <div className="max-w-[1600px] mx-auto px-4 lg:px-8 py-8 space-y-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            >
              {readyPlans.length} floor plan{readyPlans.length !== 1 ? "s" : ""} analysed
            </p>
          </div>
